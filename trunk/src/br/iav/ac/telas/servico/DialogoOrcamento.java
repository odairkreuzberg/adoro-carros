		...
		try {
			{
				...
				}
				{
					...
					labelOrcamento.setText("ORDEM DE SERVIÇO");
	...
	class FormHandle implements ActionListener, ItemListener  {
		...
		public FormHandle() {
			...
			if (servico.getCodigo() != 0) {
				...
				if (servico.getStatus().getNome().equals("Orçamento")){
					botaoOrcamento.setSelected(true);
		...
		private void editar() {
			...
			try {
				if (this.validarPecas()) {
					...
					labelAviso
							.setText("Não Existe as Peça necessarias em Estoque para efetuar este Orçamento!");
				}
		...
		private void addStatus() {
			if(botaoOrcamento.isSelected()){
				servico.setStatus((Status)status.search("Nome", "Igual", "Orçamento").getObjeto(0));
